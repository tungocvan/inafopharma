jQuery(document).ready(function($) {
	...
	removeItem();
	w366ImgAutoSortTxt();
	w366ImgAutoSortCheckbox();
});

function w366ImgAutoSortTxt(){
	jQuery(".w366-auto-sort-text").bind("keyup", function(e) {
		var eContainer = jQuery("#"+jQuery(this).data("container")); 
		saveAttachmentIds(eContainer);
	});
}

function w366ImgAutoSortCheckbox(){
	jQuery(".w366-auto-sort-checkbox").click(function(){
		var eContainer = jQuery("#"+jQuery(this).data("container")); 
		saveAttachmentIds(eContainer);
	});
}
...
function createImgBlockHtml(attachment, eContainer, is_multiple){ //alert(888); console.log(attachment);
	...
	if(hideattribute =='0'){
		...
		var arr_title_html = '<div class="attribute-it"><input type="text" data-container="'+container_id+'" class="attr-title w366-auto-sort-text" placeholder="'+jQuery(eContainer).data('title')+'" value=""></div>';
		
		var arr_desciption_html = '<div class="attribute-it"><textarea data-container="'+container_id+'" class="attr-desciption w366-auto-sort-text"  placeholder="'+jQuery(eContainer).data('desciption')+'"></textarea></div>';
		
		var arr_link_html = '<div class="attribute-it"><input type="text" data-container="'+container_id+'" class="attr-link w366-auto-sort-text" placeholder="'+jQuery(eContainer).data('link')+'" value="'+img_link+'"></div>';
		
		var arr_link_title_html = '<div class="attribute-it"><input type="text" data-container="'+container_id+'" class="attr-link_title w366-auto-sort-text" placeholder="'+jQuery(eContainer).data('link_title')+'" value=""></div>';
		
		var arr_color_html = '<div class="attribute-it"><input type="text" data-container="'+container_id+'" class="attr-color w366-auto-sort-text" placeholder="'+jQuery(eContainer).data('color')+'" value=""></div>';
		
		var arr_target_html = '<div class="attribute-it"><input data-container="'+container_id+'" type="checkbox" class="attr-target widefat w366-checkbox w366-auto-sort-checkbox" id="'+uni_id+'"><label for="'+uni_id+'" class="checkbox_label">'+jQuery(eContainer).data('target')+'</label></div>';
		
		var attr_list = '<div class="w366-attribute-list">'
							+ arr_title_html 
							+ arr_desciption_html 
							+ arr_link_html
							+ arr_link_title_html
